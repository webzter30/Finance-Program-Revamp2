...
def recategorize_full_table(db_path, table_name):
    ...
    usaa_cond = (df['Transact'].str.contains("USAA Transfer", case=False)) & (df['Amount'] == 2000.00)
...
def main_menu():
    while True:
        ...
        print("1. Rebuild database from latest CSVs  from the bank exports -only done when you have just down loaded up to date ")
        print(" bank export csv   ")
        print("2. Re-categorize existing database using updated categories.csv- this re-writes the New_one_big database for the year")
        print("3. Show uncategorized transactions only - this is to sort out categories to reduce error in the computer")
        print(" categorizing wrong !!! - THEN GO INOT THE categories.csv, make changes, then re-run option 2")
        print("4. Lookup transactions by month and category to verify if anything is categorized incorrectly")
        print("5. Display monthly and quarterly summary breakdown")
        print("6. Show all transactions marked 'LOOK INTO' for further investigation")
        print("7. Show all transactions grouped by category (detailed)/ TO MAKE SURE TRANSACATIONS ARE CATEGORIZED CORRECTLY")
        print(" 8. CASH FLOW  ")
        print("9. show_emergency_fund_estimate")
        print("10. 📆 Monthly Net Cash Flow vs Rolling Cost + Savings Target(2,500)")
        print("11. 📆 compare year 24 and 25 cash flow")
        print("12. Review category spending with comparison")
        print("13. Review transactions by date range _ like a credit card month day 9 to month day 9 etc..")
        print("7. Forecast Year-End Net Cash Flow (2025 vs 2024)")

        
        print("0. Exit")
        print("debug emergency fund estimate option 99")
        print("debug emergency fund estimate is mortage included 98")
        print("97 find mortgage rows debug!!!!!!!!!!! ")
        print("95. Emergency fund (RAW, simple category rules)")
        print("93. Audit month used in emergency fund (included vs excluded)")
        print("97. sanity check 10_19-25 NEW CSV LOADER TO MAKE THE DEBITS AND CREDITS IN ON LINE AMOUNT COLUMN")
        print("98. debug mortgage totals 10_19_2025")
        print("9.5 and 9.8 emergency fund teardown, the 9.8 (use this one) give the median/mean/and also drops outliers 9_16_25")
        choice = input("\nSelect an option (0-4): ").strip()

        if choice == '1':
            create_data_base()
        elif choice == '2':
            recategorize_full_table("ONE_BIG_ACCOUNT_combined_data2025.db", "NEW_ONE_BIG_ACCOUNT_data_2025")
        elif choice == '3':
            engine = create_engine("sqlite:///ONE_BIG_ACCOUNT_combined_data2025.db")
            df = pd.read_sql_table("NEW_ONE_BIG_ACCOUNT_data_2025", engine)
            show_uncategorized(df, n=50)
        elif choice == '4':
            lookup_by_month_and_category()
        elif choice == '5':
            display_monthly_and_quarterly_summary()
        elif choice == '6':
            show_look_into_transactions()
        elif choice == '7':
            show_all_transactions_grouped_by_category()
        elif choice == '8':
            cash_flow_by_month()

        elif choice == "9":
            df = load_main_df()
            show_emergency_fund_estimate(df)

        # default (already side-by-side)
        elif choice == "9.5":
        ...
        elif choice=="10": forecast_year_end()

        #elif choice == "10":
          #  df = load_main_df()
           # show_dynamic_surplus_vs_goals(df)
        elif choice =="11":
            compare_cash_flow_2024_vs_2025()
            print("📊 Running Year-over-Year Comparison...")

        elif choice == "12":
            year = input("Enter the year to review (e.g., 2025): ").strip()
            month = input("Enter the month to review (e.g., April): ").strip()
            db_path = f"ONE_BIG_ACCOUNT_combined_data{year}.db"
            review_monthly_expense_categories_with_comparison(db_path, year, month)

        elif choice == "13":
            review_cc_charges_between_dates()

        elif choice == "99":
            df = load_main_df()
            debug_verify_emergency_fund(df)

        elif choice == "98":
            df = load_main_df()
            debug_check_mortgage(df)
        # in main_menu() under the other debug options:
        elif choice == "97":
            quick_find_mortgage_rows()
        elif choice == "95":
            emergency_fund_from_raw()
        elif choice == "93":
            y = input("Year (blank=today.year): ").strip()
            m = input("Month number 1-12 (blank=last completed): ").strip()
            y = int(y) if y else None
            m = int(m) if m else None
            audit_emergency_month(y, m)
